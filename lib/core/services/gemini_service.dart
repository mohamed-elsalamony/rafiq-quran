...
class GeminiService {
  ...
  // قائمة الموديلات التي يتم تجربتها بالترتيب
  ...
  String _currentApiKey = _builtInApiKey;
...
""";
  ...
  Future<void> _ensureInitialized() async {
    ...
    // محاولة قراءة مفتاح API مخصص من الإعدادات
    String apiKey = _builtInApiKey;
    try {
      final prefs = await SharedPreferences.getInstance();
      final userKey = prefs.getString('gemini_api_key') ?? '';
      if (userKey.trim().isNotEmpty) {
        apiKey = userKey.trim();
        debugPrint('Using custom user Gemini API Key');
      }
    } catch (_) {}

    _currentApiKey = apiKey;
    _tryInitialize(apiKey, modelIndex: 0);
  }

  void _tryInitialize(String apiKey, {int modelIndex = 0}) {
    if (modelIndex >= _modelsToTry.length) return;
    try {
      final modelName = _modelsToTry[modelIndex];
      ...
      _isInitialized = true;
      debugPrint('GeminiService initialized successfully with $modelName using ${_usingBackup ? "backup" : "primary"} key.');
    } catch (e) {
      debugPrint('GeminiService initialization error for model ${_modelsToTry[modelIndex]}: $e');
      if (modelIndex + 1 < _modelsToTry.length) {
        _tryInitialize(apiKey, modelIndex: modelIndex + 1);
      } else if (!_usingBackup && apiKey != _backupApiKey) {
        _usingBackup = true;
        _tryInitialize(_backupApiKey, modelIndex: 0);
      }
    }
  }
  ...
  Future<String?> sendMessage(String message) async {
    ...
    if (_model != null && _chatSession != null) {
      try {
        ...
        debugPrint('Gemini API failed with model ${_modelsToTry[_currentModelIndex]}: $e');
        
        // 1. تجربة باقي الموديلات مع نفس المفتاح
        int nextIndex = _currentModelIndex + 1;
        while (nextIndex < _modelsToTry.length) {
          try {
            final modelName = _modelsToTry[nextIndex];
            debugPrint('Retrying sendMessage with model: $modelName');
            _tryInitialize(_currentApiKey, modelIndex: nextIndex);
            final response = await _chatSession!.sendMessage(Content.text(message));
            final text = response.text;
            if (text != null && text.trim().isNotEmpty) {
              _localHistory.add(Content('user', [TextPart(message)]));
            ...
            debugPrint('Retry with model $nextIndex failed: $err');
            nextIndex++;
          }
        }
        
        // 2. تجربة المفتاح الاحتياطي والموديل الأول ثم البقية
        if (!_usingBackup && _currentApiKey != _backupApiKey) {
          _usingBackup = true;
          ...
          int backupIndex = 0;
          while (backupIndex < _modelsToTry.length) {
            try {
              final modelName = _modelsToTry[backupIndex];
              debugPrint('Retrying sendMessage with backup key and model: $modelName');
              _tryInitialize(_backupApiKey, modelIndex: backupIndex);
              final response = await _chatSession!.sendMessage(Content.text(message));
              final text = response.text;
              if (text != null && text.trim().isNotEmpty) {
                _localHistory.add(Content('user', [TextPart(message)]));
              ...
              debugPrint('Retry with backup key and model $backupIndex failed: $err');
              backupIndex++;
            }
          }
        }
  ...
  List<ChatMessage> getChatHistory() {
    final List<Content> internalHistory = [];
    if (_model != null && _chatSession != null && _chatSession!.history.isNotEmpty) {
      internalHistory.addAll(_chatSession!.history);
    } else {
      internalHistory.addAll(_localHistory);
    }

    return internalHistory.map((content) {
      String text = "";
